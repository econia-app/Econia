...
function AuthModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    if (!email.includes("@") || password.length < 6) { setError(password.length < 6 ? "Mot de passe : 6 caractères minimum." : "Email invalide."); setLoading(false); return; }
    if (mode === "signup") {
      ...
      <div style={{ background: T.card, borderRadius: "16px", maxWidth: "400px", width: "100%", padding: "32px" }}>
        {success ? (
          ...
          <div style={{ marginBottom: "16px" }}>
            ...
            <input value={password} onChange={e => setPassword(e.target.value)} type="password" placeholder="6 caractères minimum" style={{ width: "100%", padding: "10px 14px", border: `1.5px solid ${T.border}`, borderRadius: "8px", fontSize: "14px", outline: "none", boxSizing: "border-box" as const }} onKeyDown={e => e.key === "Enter" && handleSubmit()} onFocus={e => (e.target as HTMLInputElement).style.borderColor = T.accent} onBlur={e => (e.target as HTMLInputElement).style.borderColor = T.border} />
          </div>
